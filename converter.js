...
function getUserProfile() {
    ...
    if (qual === 'alevels') {
        ...
        if (profile.subjects.length >= 3) {
            ...
            // Sort all grades best to worst
            const sortedGrades = Object.values(profile.grades).sort((a, b) => 
                gradeOrder.indexOf(a) - gradeOrder.indexOf(b)
            );
            
            // Store top 3 grades for 3-A-Level comparison
            profile.overallGrades = sortedGrades.slice(0, 3).join('');
            
            // If student has 4+ A-Levels, also store top 4 grades
            if (profile.subjects.length >= 4) {
                profile.overallGrades4 = sortedGrades.slice(0, 4).join('');
                profile.has4ALevels = true;
            } else {
                profile.has4ALevels = false;
            }
        }
...
function matchALevels(userProfile, course) {
    ...
    // 1. Check overall grades - use 4-A-Level requirement if student has 4 and course specifies it
    if (userProfile.has4ALevels && course.AL_Min_Grades_4 && course.AL_Min_Grades_4.trim() !== '') {
        // Student has 4 A-Levels AND course has specific 4-A-Level requirement
        if (!meetsALevelGradeRequirement(userProfile.overallGrades4, course.AL_Min_Grades_4)) {
            return { 
                match: 'unlikely', 
                reason: `Need ${course.AL_Min_Grades_4} (4 A-Levels), you have ${userProfile.overallGrades4}` 
            };
        }
    } else if (course.AL_Min_Grades) {
        // Use standard 3-A-Level requirement (or student only has 3 A-Levels)
        if (!meetsALevelGradeRequirement(userProfile.overallGrades, course.AL_Min_Grades)) {
            return { 
                match: 'unlikely', 
