...
const CONFIG = {
    ...
    requirementsRange: 'Sheet1!A:W', // Updated to include recommended columns
...
function checkSpecificGrades(gradeReqString, userProfile, requiredSubjects, isIB = false) {
    /**
     * CRITICAL: This function handles BOTH mandatory and "if taken" requirements
     * 
     * Logic:
     * 1. If subject is in requiredSubjects → MANDATORY (must have it)
     * 2. If subject is ONLY in gradeReqString → CONDITIONAL "if taken"
     *    - If student HAS subject → enforce grade
     *    - If student DOESN'T have subject → ignore (not required)
     */
    ...
    for (let req of requirements) {
        ...
        // Check if ANY of these subjects are in requiredSubjects (mandatory)
        const isMandatory = subjects.some(subject => 
            requiredSubjects && requiredSubjects.includes(subject)
        );
        
        // Check which subjects user has
        ...
        if (subjectsUserHas.length === 0) {
            ...
            if (isMandatory) {
                // Mandatory but missing → FAIL
                return { 
                    match: false, 
                    reason: `Missing required subject: ${subjects.join(' or ')}` 
                };
            } else {
                // Optional "if taken" and not taken → PASS (skip this requirement)
                continue;
            }
        }
        
        // User HAS at least one of these subjects → check grade
        ...
        if (!foundMatch) {
            // User has subject(s) but grade insufficient
            ...
            if (isMandatory) {
                ...
                // Optional "if taken" but insufficient grade → FAIL
                return { 
                    match: false, 
                    reason: `${subjects.join(' or ')} at ${requiredGrade} required if taken (you have: ${grades})` 
                };
            }
        }
    }
...
function extractRequiredSubjects(reqString) {
    /**
     * Extract all subjects from Required_Subjects string
     * Used to determine if subjects in Subject_Specific_Grades are mandatory or conditional
     */
    if (!reqString || reqString.trim() === '') {
        return [];
    }
    
    // Remove all operators and split
    const subjects = reqString
        .split(/[;|]/)
        .map(s => s.trim())
        .filter(s => s);
...
function matchALevels(userProfile, course) {
    ...
    // 3. Check specific subject grades (handles both mandatory and "if taken")
    const requiredSubjects = extractRequiredSubjects(course.AL_Required_Subjects);
    const gradeCheck = checkSpecificGrades(
        course.AL_Subject_Specific_Grades, 
        userProfile, 
        requiredSubjects,
        false
    );
...
function matchIB(userProfile, course) {
    ...
    // 4. Check specific HL subject grades (handles both mandatory and "if taken")
    const requiredSubjects = extractRequiredSubjects(course.IB_Required_HL_Subjects);
    const gradeCheck = checkSpecificGrades(
        course.IB_Subject_Specific_Grades, 
        userProfile, 
        requiredSubjects,
        true
    );
...
function matchAP(userProfile, course) {
    ...
    if (course.AP_Required_Subjects) {
        const requirements = course.AP_Required_Subjects.split(';').map(s => s.trim());
        ...
        for (let reqGroup of requirements) {
            ...
            for (let subject of subjectsUserHas) {
                if (userProfile.grades[subject] >= minGrade) {
            ...
            if (!foundQualifying) {
                ...
                return { 
                    match: 'possible', 
                    reason: `Need grade ${minGrade} in ${subjects.join(' or ')} (you have: ${grades})` 
                };
            }
        }
    }
...
1. **Updated `CONFIG.requirementsRange`** to `'Sheet1!A:W'` to include the 3 new recommended columns

2. **New function `extractRequiredSubjects()`** - Extracts all subjects from `Required_Subjects` string to determine if subjects are mandatory

3. **Updated `checkSpecificGrades()` function** - Now accepts `requiredSubjects` parameter and implements "if taken" logic:
   - If subject is in `Required_Subjects` → **MANDATORY** (fail if missing)
   - If subject is ONLY in `Subject_Specific_Grades` → **CONDITIONAL** "if taken"
     - If student HAS subject → enforce grade
     - If student DOESN'T have subject → pass (ignore requirement)

4. **Updated matching functions** (`matchALevels`, `matchIB`, `matchAP`):
   - Extract required subjects before calling `checkSpecificGrades()`
   - Pass `requiredSubjects` to `checkSpecificGrades()`

5. **Better error messages** - Distinguishes between:
   - "Missing required subject: X" (mandatory)
   - "X at grade Y required if taken" (conditional)

---

## **How It Works Now:**

### **Example 1: Latin at A if taken**
```
AL_Required_Subjects: (empty)
AL_Subject_Specific_Grades: Latin:A
```

**Student A: Has Latin at A** → ✅ PASS  
**Student B: Has Latin at B** → ❌ FAIL "Latin at A required if taken (you have: Latin (B))"  
**Student C: No Latin** → ✅ PASS (Latin not required)

### **Example 2: Chemistry required at A***
```
AL_Required_Subjects: Chemistry
AL_Subject_Specific_Grades: Chemistry:A*// Configuration
const CONFIG = {
    apiKey: 'AIzaSyBbulks6r4-Bd1baf8v-b0Fx-5zZngBlIo',
    spreadsheetId: '11l_J840GhoTlVfGormRLQhm_Qe2_SmpFibVEO5WFKik',
    requirementsRange: 'Sheet1!A:W', // Updated to include recommended columns
...
```
AL_Required_Subjects: Chemistry
AL_Subject_Specific_Grades: Chemistry:A*
